...
dynamodb = boto3.client('dynamodb', region_name='us-west-2', endpoint_url=DYNAMODB_ENDPOINT)

logging.basicConfig(level=logging.INFO, filename='fetch_secret.log', filemode='a', format='%(asctime)s - %(levelname)s - %(message)s')

def fetch_secret(codeName):
    try:
        response = dynamodb.get_item(
            TableName='devops-challenge',
            Key={'codeName': {'S': codeName}}
        )
